			...
			axios({
				...
				url: `https://verify.eryn.io/api/user/${member.id}`,
  				...
  				if (response.status = 'ok') {
					...
					member.send(embed).catch(error => { console.log(`❌ There was an error dming the user about the verification status.`) })
					...
					VerHook.send(verembed).catch(error => { console.log(`❌ There was an error sending a user's info to the webhook.`) })
  				}
